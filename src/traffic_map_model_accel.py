...
class TrafficMapModelAccel(pl.LightningModule):
    ...
    def simstep(self, last_obs, action, player_index, num_veh):
        ...
        for i in range(d_start_state.size(0)):
            ...
            obs = self.step_layer(curr_state, d_action[i], [player_index[i]], 1, 1, 30, 2, 1, 1, 1.5, 4)
            obs_array.append(obs)
            reward = self.d_compute_reward(obs).type_as(last_obs)
